...
#
#     start = time.time()
#     alignments, repair_alignments = repair.apply_with_alignments(tree, m_tree, log, alignments, Version.AR_LINEAR,
#                                                                  parameters, 1)
#     end = time.time()
#     ra_time = end - start + align_time
#
#     start = time.time()
#     alignments2, repair_alignments2 = repair.apply_with_alignments(tree, m_tree, log, alignments, Version.AR_TOP_DOWN,
#                                                                    parameters, 1)
#     end = time.time()
...
# from repair_alignment.process_tree.generation.utils import loop_two_to_three
from repair_alignment.process_tree.generation import stochastic_pt_mutate as pt_mutate
from repair_alignment.process_tree.generation import stochastic_pt_create as pt_create
from repair_alignment.process_tree.generation import non_fitting_log_create as log_create
from repair_alignment.process_tree.operation import pt_number, utils as pt_mani_utils
...
SHEET_NAME = [str(i) + "-" + str(j) for (i, j) in PT_RANGE]
pt_num, mpt_num, trace_num, non_fit_pro = 5, 1, 1, 0.2
depths = [3, 4, 5]
# depths = [3]
...
def apply_align_on_one_pt2(tree, m_tree, log):
    ...
    return [[optimal_time, optimal_cost, best_worst_cost, ra_time1, ra_cost1, grade1],
            [optimal_time, optimal_cost, best_worst_cost, ira_time2, ira_cost2, grade2],
            [optimal_time, optimal_cost, best_worst_cost, ira_time3, ira_cost3, grade3]]


def compute_align_grade1(num):
    mp_trees = pd.read_excel(m_tree_file, sheet_name=SHEET_NAME)
    # logs = pd.read_excel(log_file, sheet_name=SHEET_NAME)
    # logs = pd.DataFrame(columns=SHEET_NAME)
    # for i in SHEET_NAME:
    #     logs[i] = pd.read_csv(PATH + '/0.2/log' + i + ".csv")
    align_result = list()
    align_result2 = list()
    align_result3 = list()
    ...
    for i in mp_trees:
        ...
        align_info2 = pd.DataFrame(columns=["optimal time", "optimal cost", "best worst cost",
                                            "repair align time", "repair align cost", "grade"])
        align_info3 = pd.DataFrame(columns=["optimal time", "optimal cost", "best worst cost",
                                            "repair align time", "repair align cost", "grade"])
        for j, m_tree in enumerate(mpt_list):
            m_tree = pt_utils.parse(m_tree)
            tree = pt_utils.parse(tree_list[j])
    ...
    with pd.ExcelWriter(PATH + 'align_repair3.xlsx') as writer:
