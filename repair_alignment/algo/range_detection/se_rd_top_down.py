    ...
    while True:
        ...
        if not align_utils.is_log_move(move, True):
            if align_utils.move_in_subtree(move, tree_info[node_index].tree_range, mapping_t):
                break
            move_in_left_neighbors = neighbors_ranges.is_in_range(align_utils.move_index(move, mapping_t, True))
            if align_utils.is_sync_move(move, True) and move_in_left_neighbors:
                left_bound = cur_pos + 1
            elif align_utils.is_model_move(move, True) and move_in_left_neighbors:
...
def find_right_bound(range_interval, align, node_index, tree_info, mapping_t, neighbors_ranges):
    ...
    while True:
        ...
        if not align_utils.is_log_move(move, True):
            if align_utils.move_in_subtree(move, tree_info[node_index].tree_range, mapping_t):
                break
            move_in_right_neighbors = neighbors_ranges.is_in_range(align_utils.move_index(move, mapping_t, True))

            if align_utils.is_sync_move(move, True) and move_in_right_neighbors:
                right_bound = cur_pos - 1
            elif align_utils.is_model_move(move, True) and move_in_right_neighbors:
