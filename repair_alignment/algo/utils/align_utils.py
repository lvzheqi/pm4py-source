...
from repair_alignment.process_tree.conversion.to_petri_net import LOCK_END, LOCK_START
...
def is_log_move(move, ret_tuple_as_trans_desc=False):
    move = move[1] if ret_tuple_as_trans_desc else move
    return move[0] != align_utils.SKIP and move[1] == align_utils.SKIP


def is_model_move(move, ret_tuple_as_trans_desc=False):
    move = move[1] if ret_tuple_as_trans_desc else move
    return move[0] == align_utils.SKIP and move[1] != align_utils.SKIP
