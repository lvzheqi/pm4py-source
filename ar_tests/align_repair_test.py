...
class AlignRepairTest(unittest.TestCase):
    ...
    def test_align_repair1(self):
        ...
        a_repair = align_repair.apply(tree1, tree2, log, alignments, self.parameters)
        ...
        l_a_repair = align_repair.apply(tree1, tree2, log, l_alignments, self.parameters_tuple_false)
    ...
    def test_align_repair4(self):
        ...
        exp_l_a_repair = [{'alignment': [('>>', '1_s'), ('>>', '2_s'), ('a', 'a'), ('>>', '2_e'), ('>>', None),
                           ...
                           'visited_states': 6, 'queued_states': 6, 'traversed_arcs': 5, 'fitness': 0.5833333333333333}]
        for i in range(len(alignments)):
            self.assertEqual(a_repair[i]["cost"], exp_a_repair[i]["cost"])
            self.assertEqual(a_repair[i]["fitness"], exp_a_repair[i]["fitness"])
            self.assertEqual(exp_l_a_repair[i]["cost"], l_a_repair[i]["cost"])
