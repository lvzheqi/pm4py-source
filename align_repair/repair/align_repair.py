...
def detect_change_scope(align, subtree, trace, ret_tuple_as_trans_desc):
    ...
    while True:
        ...
        if pt_align_utils.is_node_start(align[index], subtree, ret_tuple_as_trans_desc):
            scope.index_append(index)
            new_trace = Trace()
            while not pt_align_utils.is_node_end(align[index], subtree, ret_tuple_as_trans_desc):
                if pt_align_utils.is_log_move(align[index], ret_tuple_as_trans_desc) or \
                        (pt_align_utils.check_model_label_belong_to_subtree(align[index], children,
                                                                            ret_tuple_as_trans_desc)
                         and not pt_align_utils.is_model_move(align[index], ret_tuple_as_trans_desc)):
                    new_trace.append(trace[e_index])
                e_index = e_index + 1 if not pt_align_utils.is_model_move(align[index],
                                                                          ret_tuple_as_trans_desc) else e_index
                index += 1
            scope.traces_append(new_trace)
        e_index = e_index + 1 if not pt_align_utils.is_model_move(align[index], ret_tuple_as_trans_desc) else e_index
        index += 1
    return scope
...
def alignment_reassemble(align, sub_aligns, anchor_index, subtree1, ret_tuple_as_trans_desc):
    for i in range(len(anchor_index) - 1, -1, -1):
        ...
        while True:
            ...
            if len(sub_align) <= 1:
                while not pt_align_utils.is_node_end(align[index], subtree1, ret_tuple_as_trans_desc):
                    if pt_align_utils.is_log_move(align[index], ret_tuple_as_trans_desc) or \
                            pt_align_utils.check_model_label_belong_to_subtree(align[index], labels_in_subtree1,
                                                                               ret_tuple_as_trans_desc):
                        align.pop(index)
                    else:
                        index += 1
            ...
            if pt_align_utils.is_node_end(align[index], subtree1, ret_tuple_as_trans_desc):
            ...
            if pt_align_utils.is_model_move(sub_align[0], ret_tuple_as_trans_desc):
                align.insert(index, sub_align.pop(0))
                index += 1
            elif pt_align_utils.compare_log_label(align[index], sub_align[0], ret_tuple_as_trans_desc):
                align[index] = sub_align.pop(0)
                index += 1
            elif pt_align_utils.check_model_label_belong_to_subtree(align[index], labels_in_subtree1,
                                                                    ret_tuple_as_trans_desc):
                align.pop(index)
            else:
                index += 1
